...
class Fase00(FaseBase):
    def __init__(self, largura, altura):
        ...
        self.surf["Background"] = pygame.image.load(join('images', 'FASE 0.png')).convert_alpha()
...
class Fase01(FaseBase):
    ...
    def __init__(self, largura, altura):
        ...
        self.cor_contador = pygame.Color("black")

        self.titulo = " "
        self.cor_titulo = pygame.Color("black")
        self.CoresFormas = [
        ...
        self.fonte_titulo = pygame.font.Font("PressStart2P.ttf", 50)
        self.fonte_contador = pygame.font.Font("PressStart2P.ttf", 20)

    def desenhar_titulo(self, tela):
        texto = self.fonte_titulo.render(self.titulo, True, self.cor_titulo)
        rect = texto.get_rect(center=(self.largura // 2, 50))
        tela.blit(texto, rect)
    ...
    def criar_forma(self, tipo, largura, altura):
        ...
        if tipo == "Círculo":
            return Circulo(largura, altura, random.choice(self.CoresFormas))
        if tipo == "Retângulo":
            return Retangulo(largura, altura, random.choice(self.CoresFormas))
        elif tipo == "Quadrado":
            return Quadrado(largura, altura, random.choice(self.CoresFormas))
        elif tipo == "Triângulo":
            return Triangulo(largura, altura, random.choice(self.CoresFormas))
        else:
            raise ValueError(f"Tipo de forma desconhecido: {tipo}")
    ...
    def impactar_forma(self, pos):
        for forma in self.formas:
            if not forma.impactada and forma.foi_clicado(pos):
                ...
                # se chegar a 10 cortes, volta ao menu
                if self.cortes_totais >= 10:
                    return "MENU" #ainda não está funcionando
